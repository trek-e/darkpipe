...
tags: [providers, oauth2, imap, caldav, carddav, api-integration]
dependency_graph:
  requires: [10-01 (migration-state), 10-02 (caldav-carddav-sync)]
  provides: [provider-interface, oauth2-device-flow, gmail-provider, outlook-provider, icloud-provider, mailcow-api, mailu-api, generic-provider]
  affects: [10-04 (cli-wizard)]
tech_stack:
  added: [golang.org/x/oauth2, custom-xoauth2-sasl]
  patterns: [provider-abstraction, registry-pattern, oauth2-device-flow, api-clients, wizard-prompts]
key_files:
  created:
  ...
  modified:
    ...
    - deploy/setup/go.sum
decisions:
  - Custom XOAUTH2 SASL client implementation (go-sasl only has OAUTHBEARER, not XOAUTH2)
  - Provider registry pattern with init() registration to avoid import cycles
  - OAuth2 device flow (RFC 8628) for Gmail and Outlook per locked decision
  - iCloud requires app-specific passwords (2FA prerequisite documented in wizard prompts)
  - MailCow API uses X-API-Key header authentication
  - Mailu API uses Bearer token authentication with fallback to IMAP-only
  - Generic provider supports flexible IMAP/CalDAV/CardDAV endpoint configuration
  - WizardPrompts return provider-specific setup flows for CLI integration
metrics:
  duration: 397s
  completed: 2026-02-15T02:18:00Z
  tasks: 2
  files: 12
  tests: 19
---

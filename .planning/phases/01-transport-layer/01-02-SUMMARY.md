...
plan: 02
subsystem: transport
tags: [mtls, pki, step-ca, tls, certificates, backoff, systemd]

# Dependency graph
requires:
  - phase: 01-transport-layer/01
    provides: "Go module, WireGuard config generation, deployment scripts"
provides:
...
affects: [02-cloud-relay, 09-monitoring, 01-transport-layer/03]

# Tech tracking
tech-stack:
  added: [cenkalti/backoff/v4 v4.3.0]
  patterns: [mTLS mutual auth, exponential backoff reconnection, systemd timer cert renewal, shared test utilities]
...
key-files:
  created:
    ...
    - transport/mtls/testutil/certs.go

...
tags: [multi-user, multi-domain, aliases, catch-all, stalwart, maddy, postfix, dovecot]

# Dependency graph
requires:
  - phase: 03-home-mail-server
    plan: 01
    provides: Mail server foundation for Stalwart, Maddy, and Postfix+Dovecot
provides:
...
affects: [03-03-spam-filtering, 07-build-system, 08-device-profiles]

# Tech tracking
tech-stack:
  added:
    - Stalwart REST API for user/domain/alias management
    - Maddy CLI tools (maddy creds, maddy imap-acct) for user management
    - Postfix virtual_mailbox_maps and virtual_alias_maps for multi-domain and aliases
    - Dovecot passwd-file authentication for multi-user support
  patterns:
    - User isolation by domain (alice@example.com cannot access bob@example.org mail)
    - Alias resolution before mailbox delivery
    - Catch-all with spam warnings (requires Rspamd in Plan 03)
    - Virtual mailbox domains only (no virtual_alias_domains overlap)

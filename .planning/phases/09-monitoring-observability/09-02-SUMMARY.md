...
tags: [alerting, certificates, dkim-rotation, lifecycle-management]
dependency_graph:
  requires:
    - Phase 04 DKIM selector format (dns/dkim/selector.go)
    - Phase 04 DKIM key generation (dns/dkim/keygen.go)
    - Phase 02 notification pattern (cloud-relay/relay/notify/*)
    - Phase 01 backoff library (cenkalti/backoff/v4)
  provides:
    - monitoring/alert package (multi-channel alerting with rate limiting)
    - monitoring/cert package (certificate lifecycle automation)
    - Alert trigger evaluation for four conditions
    - Certificate 2/3-lifetime renewal logic
    - Quarterly DKIM rotation automation
  affects:
    - Future monitoring daemon (Plan 09-03)
    - Certificate renewal cron jobs
    - DKIM key rotation scheduler
tech_stack:
  added:
    - crypto/x509 for certificate parsing
    - encoding/pem for PEM decoding
  patterns:
    - Fan-out dispatch (MultiNotifier pattern from Phase 02)
    - Thread-safe rate limiting with sync.Mutex
    - Exponential backoff retry (cenkalti/backoff/v4)
    - Per-alert-type deduplication (1-hour window)
    - Two-tier severity levels (warn/critical)
    - Dual-key overlap for DKIM rotation (7-day transition)
key_files:
  created:
    - monitoring/alert/notifier.go (multi-channel alert dispatch)
    - monitoring/alert/ratelimit.go (per-type rate limiting)
    - monitoring/alert/triggers.go (alert evaluation logic)
    - monitoring/cert/watcher.go (certificate expiry monitoring)
    - monitoring/cert/rotator.go (automated renewal with retry)
    - monitoring/cert/dkim.go (quarterly DKIM rotation)
    - monitoring/cert/reload.go (service reload orchestration)
  modified: []
decisions:
  - Alert rate limiting uses simple map-based implementation (not RussellLuo/slidingwindow) for single-process use case
  - Email channel uses sendmail command (assumes Postfix available on mail server)
  - Webhook channel uses curl command (reusing cloud-relay pattern)
  - CLI alerts written to NDJSON file for CLI consumption (/data/monitoring/cli-alerts.json)
  - Certificate renewal uses 2/3-lifetime rule (auto-handles Let's Encrypt 90→45 day transition)
  - Exponential backoff with 3 retries for transient failures
  - Permanent error detection (ACME issues, CAA records, domain validation failures)
  - DKIM rotation reuses Phase 4 selector format ({prefix}-{YYYY}q{Q})
  - Service reload uses hot reload (postfix reload, caddy reload) to avoid interruption
metrics:
  duration: 450s
  tasks_completed: 2
  files_created: 12
  tests_added: 5
  completed_at: 2026-02-14T16:28:13Z
---
